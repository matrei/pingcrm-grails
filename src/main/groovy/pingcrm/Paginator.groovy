...
class Paginator implements Validateable {
    ...
    int getMaxOrDefault() {
        Math.min(max ?: defaultMax, fetchMax)
    }
    ...
    Map getQueryParams() {
        [max: maxOrDefault, offset: offsetOrDefault]
    }

    Map queryParamsAnd(Map extra) {
        queryParams + extra
    }
    ...
    Map paginate(List data, Map currentRequestParams, Map paginationData = [:]) {
        ...
        def params = currentRequestParams instanceof TypeConvertingMap ?
                currentRequestParams :
                new TypeConvertingMap(currentRequestParams)

        if (pgData.total == null) {
            throw new IllegalArgumentException('paginationData is missing required entry [total]')
        }
        ...
        if (!pgData.boolean('omitPrev')) {
            ...
            links << [
                    active: false,
                    label: label,
                    url: (currentStep > firstStep) ?
                            linkGenerator.link(params: linkParams) :
                            null
            ]
        }
        ...
        if (stepsEnabled && lastStep > firstStep) {
            ...
            if (beginStep > firstStep && !pgData.boolean('omitFirst')) {
                ...
                links << [
                        active: false,
                        label: firstStep.toString(),
                        url: linkGenerator.link(params: linkParams)
                ]
            }
            //show a gap if begin step isn't immediately after first step, and if were not omitting first or prev
            def gapLabel = (pgData.gap ?:
                    messageSource.getMessage(
                            'paginate.gap',
                            null,
                            messageSource.getMessage(
                                    'default.paginate.gap',
                                    null,
                                    '..',
                                    locale
                            ),
