...
class UserCrudSpec extends ContainerGebSpec {
    ...
    void 'it is allowed to delete a user that is not the demo user'() {
        given: 'a logged in demo user'
    ...
    void 'users can be created with valid input'() {
        ...
        and: 'submits valid user details'
            userCreatePage.createUser(
                    'Han',
                    'Solo',
                    'han@solo.com',
                    'secret',
                    'src/integration-test/resources/assets/avatar.jpg' as File
            )
    ...
    void 'users cannot be created with a too large photo file size (server-side validation: #testServerMaxUploadSizeValidation)'() {
        when: 'a demo user is logged in'
            loginDemoUser()
        ...
        and: 'submitting the form with a photo that has a too large file size'
            userCreatePage.createUser(
                    ...
                    'josh@doe.com',
                    'secret',
                    'src/integration-test/resources/assets/avatar-filesize-to-large.jpg' as File
            )

        then: 'an error message is shown'
            userCreatePage.flashError == 'There is one form error.'
    ...
    void 'a user can upload an avatar image'()  {
        when: 'a demo user is logged in'
            loginDemoUser()
        ...
        and: 'the avatar image is shown'
            def avatarImage = userEditPage.avatarImage
            waitFor {
                js.exec(
                        avatarImage,
                        'return arguments[0].complete'
    ...
    void 'the user cannot upload files larger than allowed (server-side validation: #testServerMaxUploadSizeValidation)'(boolean testServerMaxUploadSizeValidation) {
        ...
        and: 'trying to upload an image file that is to big'
            userEditPage.photoField.file = 'src/integration-test/resources/assets/avatar-filesize-to-large.jpg' as File
            userEditPage.save()

        then: 'an error message is shown'
            userEditPage.flashError == 'There is one form error.'
